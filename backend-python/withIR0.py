...
ESP32_CAM_URL = 'http://192.168.204.73/cam-hi.jpg'
...
try:
    ...
    esp32_base_url = f"{parsed_url.scheme}://{parsed_url.netloc}"
    beep_url = f"{esp32_base_url}/beep"
    print(f"ESP32 Base URL: {esp32_base_url}")
    print(f"ESP32 Beep URL: {beep_url}")
except Exception as e:
    print(f"!! Error parsing ESP32_CAM_URL: {e}")
    print("   Cannot determine beep URL. Exiting.")
    exit()
    ...
    try:
        ...
        if detected_qr_data:
            ...
            if detected_qr_data != last_sent_qr_data or (current_time - last_send_time > DEBOUNCE_SECONDS):
                ...
                try:
                    print(f"Sending BEEP request to {beep_url}")
                    beep_response = requests.get(beep_url, timeout=BEEP_TIMEOUT)
                    beep_response.raise_for_status()
                    print(f"--> Beep request successful ({beep_response.status_code})")
                except requests.exceptions.Timeout:
                    print(f"!! Timeout sending BEEP request to {beep_url}")
                except requests.exceptions.RequestException as beep_err:
                    print(f"!! Error sending BEEP request: {beep_err}")
                ...
                try:
                    ...
                    print(f"--> Backend POST Success! Response: {response.status_code} - {response.text}")
                    ...
                    if http_err.response is not None and http_err.response.status_code == 404:
                         ...
                         print(f"   Backend returned status: {http_err.response.status_code if http_err.response is not None else 'Unknown'}")
                         last_sent_qr_data = detected_qr_data
                         last_send_time = current_time

                except requests.exceptions.ConnectionError as conn_err:
                     print(f"!! Connection Error sending POST to backend: {conn_err}")
                except requests.exceptions.Timeout:
                    print(f"!! Timeout sending POST request to backend.")
                except requests.exceptions.RequestException as req_err:
                    print(f"!! General Request Error sending POST: {req_err}")
                except Exception as e:
                     print(f"!! An unexpected error occurred during POST sending: {e}")

        cv2.imshow("ESP32 CAM QR Scanner", frame)

    except urllib.error.URLError as e:
        print(f"!! Network Error fetching image from ESP32 ({ESP32_CAM_URL}): {e}")
        print("   Check ESP32 IP address, Wi-Fi connection, and if the ESP32 is running.")
        time.sleep(2)
    except cv2.error as e:
        print(f"!! OpenCV Error processing image: {e}")
        time.sleep(1)
    except requests.exceptions.Timeout:
         print(f"!! Timeout connecting to ESP32 CAM at {ESP32_CAM_URL}")
         time.sleep(2)
    except Exception as e:
